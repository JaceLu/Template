(function (factory) {
    ...
    _e(function (E, $) {
        ...
        E.fn.saveSelection = ieRange ? emptyFn : function (range) {
            var self = this, _parentElem, selection, txt = self.txt.$txt.get(0);
            if (range) {
    ...
    _e(function (E, $) {
        ...
        Txt.fn.init = function () {
            ...
            if ($valueContainer.get(0).nodeName === 'DIV') {
                ...
                $txt = $('<div class="wangEditor-txt" contentEditable="true">' +
                    currentValue +
                    '</div>');
            }
    ...
    _e(function (E, $) {
        ...
        E.createMenu(function (check) {
            ...
            $btnSubmit.click(function (e) {
                e.preventDefault();
                var map = mapData.map, isDynamic = $check.is(':checked'), markers = mapData.markers, center = map.getCenter(), centerLng = center.lng, centerLat = center.lat, zoom = map.getZoom(), size = map.getSize(), sizeWidth = size.width, sizeHeight = size.height, position, src, iframe;
                if (isDynamic) {
                    //动态地址
...
$(function () {
    /*
      wangEditor生成编辑器时，将根据传入的div或者textarea的高度来设置编辑器的高度，宽度只能通过div或者textarea的父容器来控制。
      可以通过div或者textarea的max-height来控制编辑区域的最大高度，此功能可用可不用，只支持识别max-height的浏览器。
      ...
      如果要实时监控编辑器内容的变化，可通过 editor.onchange 来处理。注意，该操作必须在执行 editor.create() 方法之前。

      赋值操作可以通过editor.$txt.html()来实现
        */
    //wangEditor富文本
    function initWang($editor, set) {

        var h = $editor.data("height") || "500px";
        var settings = $.extend({
            height: h,
            uploadImgUrl: "/api/WangUp/UploadFile",
            defaultText: ""
        }, set);
        var editor = $editor.data("wangEditor");
        if (editor)
            return editor;
        editor = new wangEditor($editor);
        $editor.css({ height: settings.height });
        $editor.data("wangEditor", editor);
        editor.config.uploadHeaders = { 'Accept': 'text/x-json' };
        editor.config.uploadImgUrl = settings.uploadImgUrl;
        editor.config.menus = $.map(wangEditor.config.menus, function (item, key) {
            var items = ['video', 'insertcode', 'fullscreen'];
        ...
        editor.onchange = function () {
            editor.$txt.trigger("change", this);
            $editor.trigger("change");
        };
        editor.create();
        //editor.$txt.on("keyup keydown", function () {
        //    $editor.trigger("change");
        //});
        editor.$txt.html(settings.defaultText);
        return editor;
    }
    $.fn.initWang = function (set) {
        return initWang($(this), set);
    };
    function registerWang() {
        var field = "item";
        var selector = "textarea.richedit";
        var act = function ($form, callback) {
            $form.find(selector).each(function (i, o) {
                var $o = $(o);
                var wang = $o.initWang();
                var item = $o.data(field);
                if (callback)
                    callback(wang, item);
            });
        };
        $("body").on("after.InitForm", function (sender, form) {
            var $form = $(form);
            act($form);
        }).on("after.Binder", function (sender, form, model) {
            var $form = $(form);
            act($form, function (wang, ff) {
                if (ff) {
                    wang.$txt.html(model[ff]);
                }
            });
        }).on("after.ResetForm", function (sender, form) {
            var $form = $(form);
            act($form, function (wang) {
                wang.$txt.html("");
            });
        });
    }
    $.registerWang = registerWang;
    //异步注册会导致EditAct,Add等初始化失败
    //$(function () {
    //    registerWang();
    //});
});
